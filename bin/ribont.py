...
def OffSetExtract (bamFile, startCodon, pre, logout, pathBedtools='', pathSamtools=''):
	...
	if not pathBedtools:
		...
		if out != 0:
			sys.stdout.write("ERROR: bedtools is not an excutable command, please provide the path to bedtools using the option of --bedtools")
			sys.exit()
	else:
		os.system(pathBedtools +'/bamToBed -bed12 -i '+bamFile+' > '+pre+'.bed 2>>' + logout)

	if not pathSamtools:
		out = os.system('samtools view -h -s 1.03 '+bamFile+' 2>>' + logout +'|samtools view -bS - > '+pre+'.sub.bam 2>>' + logout)
		if out != 0:
			sys.stdout.write("ERROR: samtools is not an excutable command, please provide the path to bedtools using the option of --samtools")
			sys.exit()
	else:
		os.system(pathSamtools +'/samtools view -h -s 1.03 '+bamFile+' 2>>' + logout + '|' + pathSamtools + '/samtools view -bS - > '+pre+'.sub.bam 2>>' + logout)
	
	if not pathBedtools:
		os.system('bamToBed -bed12 -i '+pre+'.sub.bam > '+pre+'.sub.bed 2>>' + logout)
		os.system('windowBed -w 100 -sm -b '+pre+'.sub.bed -a '+startCodon+' 2>>' + logout +'|cut -f 7-18|sort -k1,1 -k2,2g|closestBed -s -a stdin -b '+startCodon+' > '+pre + ' 2>>' + logout)
	else:
		os.system(pathBedtools +'/bamToBed -bed12 -i '+pre+'.sub.bam > '+pre+'.sub.bed 2>>' + logout)
		os.system(pathBedtools +'/windowBed -w 100 -sm -b '+pre+'.sub.bed -a '+startCodon+ ' 2>>' + logout + '|cut -f 7-18|sort -k1,1 -k2,2g|' + pathBedtools + '/closestBed -s -a stdin -b '+startCodon+' > '+pre + ' 2>>' + logout)
